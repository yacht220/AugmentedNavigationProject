...
public class Header {
	...
	public void process(byte[] bytes) {
		...
		System.arraycopy(bytes, indexh, h1, 0, h1.length); // 根据Minimap Service V1.doc的定义，将原始字节数组的某一部分拷贝至新的字节数组中，该部分数据为需要的值
		setMark(HttpClientUtil.getInt(h1)); // 将新的字节数组内容转换成需要的数据类型
		indexh += h1.length; // 索引跳至下一个位置
