...
public class ARUtil {
	...
	private boolean isFirstTimeLocation; // 第一次搜索到地理位置
	private int locationUpdateTime;//地理位置更新时间
	
	/* 传感器 */
	private float prevals = 0; // 前一个传感器方向读数
	...
	private class CoordGps2CoordDeflectTask extends AsyncTask<Double, Void, Void>{
		@Override
		protected Void doInBackground(Double... params) {
			// TODO Auto-generated method stub
			myLocationCoord20 = ms.coordGps2Coord20(new CoordGps(params[0], params[1]));
	...
	private synchronized void updateLocationCoord() {
		// 第一次搜索到地理位置转换时先直接转换，之后就在doInBackground()中进行位置转换
		if(isFirstTimeLocation == true){
			myLocationCoord20 = ms.coordGps2Coord20(myLocationCoordGps);
			myLocationCoordDeflect = ms.coord202CoordDeflect(myLocationCoord20, false);	
			isFirstTimeLocation = false;
		}else{
			Double[] params = {myLocationCoordGps.getX(), myLocationCoordGps.getY()};
			coordGps2CoordDeflectTask = new CoordGps2CoordDeflectTask();
